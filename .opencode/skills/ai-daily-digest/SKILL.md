...
如果配置存在且有 `deepseekApiKey`，询问是否复用：
...
question({
  questions: [{
    ...
    question: "需要 DeepSeek API Key 进行 AI 评分和摘要\n\n获取方式：访问 https://platform.deepseek.com/api_keys 创建免费 API Key",
    options: []
  }]
})
```

如果 `config.deepseekApiKey` 已存在，跳过此步。
...
- 网络访问（需要能访问 RSS 源和 DeepSeek API）
